...
目前使用中国石化长城SN级别的`5w-30`的全合成机油，在中国石化加油站内购买，不在网上购买，线上何线下的产品也不会一样。机油的选择，首先要选对型号，然后是一定要正品，买到劣质机油会对发动机造成直接损坏。目前坚持5000-6000公里更换机油，机滤。空气滤和空调滤都是自己购买后更换。

**2026年8月的重新选择**

最初用的`4s`店的机油，后来用过中国石化加油站销售的的全合成`5w30`机油，也用过恩渡的`c2 5w30`机油,第7个年头了，今年发现有少量机油消耗的现象，后来在3000多公里就少量补加了点机油，大概不到0.1升。后来买了恩渡的`a5/b5 5w30`,咨询了`deepseek`专家版，说这个机油不适合维特拉`1.4t`发动机。在选机油的过程中，咨询`deepseek`专家版，居然给出和以前相反的结论，不能再用`c2 5w30`的机油了，要用`a3b4 5w30的机油`,没想到`a3b4 5w30`机油还不好买，大部分是`c3  5w30`,因为现在的新车大都是带有颗粒捕捉器的，这种车子适合用中低灰分度的`acea c`系列机油，所以适应市场的大部分是`acea  c`系列机油。不带颗粒捕捉器的车子，适合用`acea  a/b`系列的机油，维特拉`1.4t`发动机的`hths`数值要求>=3.5,所以在`a/b`系列的机油中最好是`a3b4的5w30`机油,在[天猫国际自营全球超级店](https://shop123971619.taobao.com/)买到了壳牌的`a3/b4 5w30 sp`机油,试试这个机油看发动机状态则么样。如果彻底买不到`a3/b4 5w30`的机油，只能选择备选的`c3 5w30`机油,看论坛上有车友也使用这个机油。嘉实多也有`a3b4 5w30`的机油（`Castrol`嘉实多极护全合成机油`5W-30` `A3/B4 SL`），不过暂时就用壳牌的。

```
A/B 系列：高灰分，适合没有颗粒捕捉器的车；
  A3/B4：高 HTHS，≥3.5，适合涡轮/高负荷；
  A5/B5：低 HTHS，偏节能，适合低负荷；
C 系列：低灰分，适合带颗粒捕捉器的车
  C3：高 HTHS，≥3.5，适合涡轮；
  C2：低 HTHS，偏节能，油膜弱
```
...
现在是在修理店更换机油和机油滤芯，如果有机会自己更换的话，首先要选对工具（14号套筒拆卸放放油螺丝，902（66-67`mm`）14牙扳手拆卸机油滤芯），还有专门的汽车保养坡道(毕竟自己没有那么合适的保养坡道)，简要换油过程如下：
...
2，拆放机油螺丝并安装好
 ...
 14号套桶拆放油螺丝，使用接油容器来盛放旧机油

3，拆旧机油滤

  推荐使用套帽式油滤扳手（902 14牙），拆卸过程中使用塑料袋包裹住旧的机油滤芯，防止机油洒漏

4，安装新的机油滤芯

向新机油滤芯加如少许新机油，并在密封上抹一些机油；用手拧上新的机油滤芯并拧紧，再使用工具多拧3/4圈

5，加注新机油3升 (自己现在买的都是1升装的机油)

6、第二天行车前检查机油位置，如果3升少点，可再少加点，让机油位置保持中间或者中间微上
```
...
目前车子已经跑了5万多公里，计划到第六年车检之后，更换倍耐力的215/55  `R17`的轮胎。这里的参数中，215指的轮胎的胎面宽度，55指的是扁平比，R是代表子午轮胎，17是指配置17寸的轮毂。关于扁平比，看下面的计算公式：
...
由于`1.4t`的四驱`Vitara`是一个偏运动的小型`Suv`，整个车子调教风格偏硬朗，车身刚性好，动力强劲，提速快，操控好，车身灵活，还省油，质量稳定（目前0故障，后续看看能达到多少公里稳定行驶）。这5万多公里开下来，最直观的感受就是好开，省心、动力强劲，四驱加持后，下雪天开着很爽，唯一的遗憾是没有条件更换雪地轮胎，体验一下装上雪地胎的vitara是什么感觉。毕竟这里不是经常下雪，更换次数多了，会损伤固定轮胎的零件，轮胎无小事，再好的车子和动力，最终都体现在轮胎上。
...
> 可以换。但换大轮后，会造成仪表里程计数不准。维特拉原是215/55 `R17`轮胎，换215/60` R17`轮胎后，由于车轮半径增大，使底盘约升高1cm；仪表表显里程和时速比原来小约3%，即原轮胎仪表显示`100km/h`的对应速度换后只显示为约97km/h；原仪表显示里程100km，换后只显示为约97km。换后的里程计数偏差可以叫4S电脑设定校正。也可以无视，因为只要心中有数知道显示里程比实际里程小3%便可；而时速显示由于原厂设定仪表显示速度比实际时速大约5%，换轮胎后的仪表显示时速反而更接近实际时速。换高宽比更大的215/60 `R17`轮胎后，由于轮胎侧壁宽度增大，会降低轮胎的抗侧扭性能，由于轮半径变大，轮缘的最大推力略有减小。体现为车的加速性能和操控性能略有下降，但舒适性有所提高。


> 有不少车友直接更换215/60 `R17`的轮胎，静音和舒适性提高不少。自己更喜欢目前的操控，好开灵活，提速快，为了保持原车的调教，目前决定还是更换原车的轮胎，我更喜欢灵活的操控，还是坚守原车的调教。等到年龄大了，可以考虑更换215/60` R17`的轮胎，提升舒适性，计划把这台vitara一直开下去。

##### 防冻液更换

目前在2023年5万公里时在原`4s`店更换过一次防冻液，以后可以每2年更换一次。除了给发动机降温之外，防冻液里有多重防锈剂，用来防止发动机的金属部件上锈，所以要定期更换。按照`4s`店更换防冻液的包装盒子品牌购买就可以。
...
| 检测项目     | 全时四驱车的适用方法 | 核心原理                                                     |
| :----------- | :------------------- | :----------------------------------------------------------- |
| **刹车检测** | **平板式制动检测台** | 低速驶上平板，在车轮滚动时踩刹车，利用车身**惯性**推平板测制动力。安全、准确、无损伤。 |
| **尾气检测** | **双怠速法**         | 原地分别测**高怠速**（约2500转）和**普通怠速**下的尾气，通过CO和`HC`浓度判断燃烧状况。 |

**为什么不能用滚筒**

**滚筒会强行制造前后轴巨大转速差，损坏中央差速器。如果损坏了后轮差速器，修理车子的费用是自己的，所以一定要使用正确的检测方法。**
...
### 涡轮增压发动机清洁剂（结论来自`DeepSeek`）


* 采用长期的保守养护而不是一次性的集中清洁，如果车子的积碳很多，一次性用量过大会导致积碳颗粒杂质可能非常多，燃烧不充分的话会动发动机造成伤害。
* 每5000公里加半瓶雪佛龙，剩下的半瓶雪佛龙拧紧盖子放在阴凉处， PEA（聚醚胺）本身是一种非常稳定的化学物质，不易分解或失效，一年之内不会过期。
* **在更换新机油后，务必等到行驶2500公里左右再加那半瓶PEA，不要在刚换完机油后立即添加PEA**

 为什么时机如此重要？

这背后的原理是我们反复强调的 **“清洗-承载-排出”闭环**。让我们再清晰地复盘一下：

1. **今天换新机油后立即添加PEA（错误做法）：**
   - **步骤：** 新机油 → 立即加PEA → PEA开始清洗积碳 → 积碳微粒进入机油。
   - **后果：** 这些被污染的机油**将在您的发动机里循环整整5000公里**。虽然新机油有能力承载，但这些杂质会持续增加机油的污染度和磨损.
2. **行驶到2500公里时添加PEA（正确做法）：**
   - **步骤：** 新机油 → 正常行驶2500公里（机油处于黄金时期）→ 加PEA → PEA清洗积碳 → 积碳微粒进入机油 → 继续行驶2500公里 → 机油在完成“承载杂质”的使命后，**被连同这些杂质一起排掉**。
   - **好处：** 这样操作，被污染的机油只运行了2500公里就被换掉了，**完美利用了机油的寿命周期，没有让新机油受委屈**。



**关于进气门积碳**：请理解，PEA添加剂对于**喷油嘴、活塞顶部和燃烧室**的积碳清洁效果非常好。但对于缸内直喷发动机的**进气门背面**，由于汽油不经过那里，PEA无法直接清洗。如果未来某天出现明显的怠速抖动、动力下降，可能需要考虑专业的"核桃砂清洗"
