...
> 运动模式(SPORT)是在自动模式（AUTO）的基础上，油门响应更敏捷，前后轮扭矩分配介入更积极，适合多弯多坡的路况进行比较激烈的运动驾驶的控制程序，可以进行长时间的使用。长期用运动模式也可以，但油耗要高一些。有些锋驭车主就长期用运动模式驾驶，也没有反映什么问题出现。
...
> sport模式和snow模式只不过是在auto模式程序的基础上，系统控制介入更积极、执行强度更大的程序而已。只有用LOCK锁定中差后，前后轴才有各50%的固定比例动力分配
...
VITARA的ALLGRIP全时四驱系统使用中央差速锁来连接前后轴，智能控制联结扭矩，在前后轴分配动力。在自动模式下，前轴分配95%的动力，后轴分配5%的动力。根据驾驶者切换sport、snow、snow+lock模式来智能分配前后轴动力，前轴（前轮）动力分配范围在95%-50%之间，后轴（后轮）动力分配范围在5%-50%之间。只有在snow+lock模式下，前后轴的动力分配达到50:50。根据不同的应用场景，山路可以使用sport模式，长期使用也没问题。雪地可以使用snow模式，长期使用也没问题。在snow+lock模式下，车速要降低在60km/h以下，车速高于60km/h就会自动解除锁定。另外只有snow+lock模式下才能使用陡坡缓降。
